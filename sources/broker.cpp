        ...
        if (fs::is_directory(x.path())) {
            for (const fs::directory_entry& y:
                ...
                if (y.path().extension() == ".txt" &&
                    ...
                    accounts.insert(
                        y.path().stem().string().substr
                        (AccountPosition, PositionLength));
                }
            }
        }
    }
...
std::stringstream Broker::PrintAccounts() const {
    ...
    for (const auto& x : accounts) {
        ...
        for (const auto& y : paths) {
            if (x == y.stem().string().substr
                   (AccountPosition, PositionLength)) {
                if ((n < 1) || (y.stem().string().substr
        ...
        ss << "broker:" << max.parent_path().filename().string() << " account:"
           ...
           << " files:" << n << " lastdate:"
           << max.stem().string().substr
              (TimePosition, PositionLength) << std::endl;
    }
    return ss;
}
